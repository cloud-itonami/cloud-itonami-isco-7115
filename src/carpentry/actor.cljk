(ns carpentry.actor
  "CarpentryPracticeActor — the ISCO-08 7115 community carpentry actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one carpentry operation request
  ...
  The unconditional invariant: the CarpentryPracticeAdvisor can never
  directly commit a record the CarpentryPracticeGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [carpentry.advisor :as advisor]
            [carpentry.governor :as governor]
            [carpentry.store :as store]))

(defn build-graph
  "Build a compiled CarpentryPracticeActor graph. `store` implements
  `carpentry.store/Store`. `advisor` implements
  `carpentry.advisor/Advisor` (defaults to `mock-advisor`).
